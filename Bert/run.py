from data_utils import BERT_Dataset, collate_fn, DataProcessor, load_embedding_dict
import torch
...
def main():
    ...
    parser.add_argument('--model_type', default='BertDPCNN', type=str, help='Bert, BertCNN, BertRNN, BertRCNN, BertDPCNN')
    parser.add_argument("--model_name_or_path", default='bert-base-chinese', type=str, help="Path to pre-trained model or shortcut name selected in the list: ")
    ...
    train_dataset = BERT_Dataset(args, 'train')
    dev_dataset = BERT_Dataset(args, 'dev')
    test_dataset = BERT_Dataset(args,'test')

    logger.info("  Num examples of Train = %d", len(train_dataset))
    logger.info("  Num examples of Dev = %d", len(dev_dataset))
    logger.info("  Num examples of Test = %d", len(test_dataset))
