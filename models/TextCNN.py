...
class Config():
    def __init__(self,data_path,embedding):
        ...
        self.dev_path = data_path + '/dev.txt'                                    # 验证集
        self.test_path = data_path + '/test.txt'                                  # 测试集
        self.class_list = [x.strip() for x in open(
            data_path + '/class.txt', encoding='utf-8').readlines()]              # 类别名单
        ...
        self.embedding_pretrained = torch.tensor(
            np.load(data_path + '/' + embedding)["embeddings"].astype('float32'))\
